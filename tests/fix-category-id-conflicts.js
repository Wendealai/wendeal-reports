  ...
  try {
    ...
    for (const [oldId, newId] of Object.entries(STANDARD_CATEGORY_MAPPING)) {
      ...
      if (reportsWithOldId.length > 0) {
        console.log(
          `📝 将 ${reportsWithOldId.length} 个报告从 ${oldId} 迁移到 ${newId}`,
        );
    ...
    for (const categoryData of standardCategories) {
      try {
        ...
        console.log(
          `✅ 标准分类 ${categoryData.name} (${categoryData.id}) 已确保存在`,
        );
      } catch (error) {
    ...
    finalCategories.forEach((category) => {
      console.log(
        `  - ${category.name} (${category.id}): ${category._count.reports} 个报告`,
      );
    });
    ...
    const orphanedReports = allReports.filter(
      (report) =>
    ...
    if (orphanedReports.length > 0) {
      console.log(
        `发现 ${orphanedReports.length} 个孤立的报告，将其移至未分类:`,
      );
