...
export async function measureQuery<T>(
  ...
  try {
    ...
    console.log(
      `Query "${queryName}" completed in ${queryTime}ms, returned ${resultCount} results`,
    );
...
function generateOptimizationSuggestions(
  ...
  if (queryTime > 1000) {
    suggestions.push(
      "Query took over 1 second - consider adding indexes or optimizing the query",
    );
  }

  if (queryTime > 500 && resultCount > 100) {
    suggestions.push(
      "Large result set with slow query - consider pagination or filtering",
    );
  }

  if (resultCount > 1000) {
    suggestions.push(
      "Very large result set - implement pagination to improve performance",
    );
  }

  if (queryTime > 200 && resultCount < 10) {
    suggestions.push(
      "Slow query with few results - check if proper indexes are in place",
    );
  }
...
export async function cachedQuery<T>(
  ...
  if (enabled && metrics.queryTime < 5000) {
...
export const optimizedQueries = {
  ...
  async getFileStats() {
    ...
    return cachedQuery(
      async () => {
        const [totalFiles, totalSize, compressedFiles, avgCompressionRatio] =
          await Promise.all([
            prisma.file.count(),
        ...
        return {
          ...
          compressionPercentage:
            totalFiles > 0 ? (compressedFiles / totalFiles) * 100 : 0,
        };
      },
      cacheKey,
  ...
  getRecommendations(): string[] {
    ...
    if (stats.size > 100) {
      recommendations.push(
        "Cache size is large, consider reducing TTL or implementing cache eviction",
      );
    }
