 ...
 */
export async function sanitizeAndOptimizeHtml(html: string, options: OptimizationOptions = {}): Promise<string> {
  ...
  if (DOMPurify && DOMPurify.sanitize) {
    try {
      ...
      console.warn('DOMPurify sanitization failed, using original content:', error);
      sanitized = html;
    }
  }
 ...
 */
export async function compressContent(content: string, compressionLevel: number = 6): Promise<Uint8Array | null> {
  await loadDependencies();
 ...
 */
export async function decompressContent(compressedData: Uint8Array): Promise<string | null> {
  await loadDependencies();
...
export async function optimizeFileContent(
  ...
  if (shouldCompress) {
    try {
      const compressed = await compressContent(optimizedContent, opts.compressionLevel);
      if (compressed) {
...
export async function calculateOptimizationSavings(
  ...
  try {
    const compressed = await compressContent(minified, options.compressionLevel || 6);
    if (compressed) {
