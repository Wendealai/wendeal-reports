...
const requestWithRetry = async <T>(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      return await performanceLogger.measure(`API Request: ${endpoint}`, async () => {
...
export const reportsApi = {
  ...
  async getById(id: string): Promise<ReportResponse> {
    ...
    return requestWithRetry<ReportResponse>(`/reports/${encodeURIComponent(id)}`);
  },
  ...
  async create(data: CreateReportRequest): Promise<ReportResponse> {
    ...
    if (data.title.length > 200) {
      throw createError.validation('Title must be less than 200 characters', 'title');
    }
  ...
  async delete(id: string): Promise<{ message: string }> {
    ...
    return requestWithRetry<{ message: string }>(`/reports/${encodeURIComponent(id)}`, {
      method: 'DELETE',
    });
  },
};
...
export const categoriesApi = {
  ...
  async getById(id: string): Promise<CategoryResponse> {
    ...
    return requestWithRetry<CategoryResponse>(`/categories/${encodeURIComponent(id)}`);
  },
  ...
  async create(data: CreateCategoryRequest): Promise<CategoryResponse> {
    ...
    if (data.name.length > 100) {
      throw createError.validation('Category name must be less than 100 characters', 'name');
    }
  ...
  async delete(id: string): Promise<{ message: string }> {
    ...
    return requestWithRetry<{ message: string }>(`/categories/${encodeURIComponent(id)}`, {
      method: 'DELETE',
    });
  },
};
...
export const tagsApi = {
  ...
  async create(data: { name: string; color?: string }): Promise<{ tag: TagData; message: string }> {
    if (!data.name || data.name.trim().length === 0) {
      throw createError.validation('Tag name is required', 'name');
    }
    
    if (data.name.length > 50) {
      throw createError.validation('Tag name must be less than 50 characters', 'name');
    }
