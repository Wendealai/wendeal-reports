...
export default function DebugPage() {
  const testLocalStorage = () => {
    try {
      ...
      alert(`localStorage测试: ${result === 'working' ? '✅ 正常' : '❌ 异常'}`);
    } catch (error) {
  ...
  const setTokenAndGo = () => {
    try {
      ...
      localStorage.setItem('user_info', JSON.stringify({id: 'debug', name: '调试用户'}));
    ...
    <div style={{
      ...
      <div style={{
        ...
        <ol style={{ color: '#6b7280', lineHeight: '1.6' }}>
          <li><strong>首先</strong>：点击"清除所有数据并刷新"清理环境</li>
          <li><strong>然后</strong>：点击"设置Token并跳转仪表板"测试完整流程</li>
          <li><strong>如果失败</strong>：依次测试localStorage和重定向功能</li>
          <li><strong>查看控制台</strong>：按F12打开开发者工具查看错误信息</li>
        </ol>
      </div>
    </div>
  );
